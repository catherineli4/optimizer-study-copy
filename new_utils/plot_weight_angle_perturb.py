#!/usr/bin/env python3
"""Plot Gaussian **weight**-perturb curves for each adamw↔muon angle-bin group.

One plot set per angle bin: mean / ΔNLL vs γ (std = γ·‖W‖_F/√n) with
adamw vs muon. Mirrors plot_logit_angle_perturb.
"""
...
DEFAULT_GCS_DIR = (
    "gs://cmu-gpucloud-catheri4/Optim-60M-tuning/WeightAngleBinPerturbEvaluation"
)
DEFAULT_OUT_DIR = os.path.join(REPO_ROOT, "results", "weight_angle_perturb")
...
def list_files(gcs_dir: Optional[str], local_dir: Optional[str]) -> List[str]:
    if local_dir:
        return sorted(
            ...
            if n.endswith("-weight_angle_bin_perturb.json")
        )
    ...
    return [
        ...
        if line.strip().endswith("-weight_angle_bin_perturb.json")
    ]
...
def collect(paths: List[str]) -> List[dict]:
    ...
    for path in paths:
        ...
        if "gammas" not in data and "sigmas" in data:
            data["gammas"] = data["sigmas"]
...
def _delta(mean_nll, baseline):
    return np.asarray(mean_nll, dtype=float) - float(baseline)


def plot_one_bin(rows, bin_label, out_dir, *, degradation, xmax, markersize=1.0):
    usable = []
    for r in rows:
    ...
    for i, r in enumerate(usable):
        ...
        chin = r["chinchilla"]
        b = r["bins"][bin_label]
        gammas = np.asarray(r.get("gammas") or r["sigmas"], dtype=float)
        for opt in ("adamw", "muon"):
            ...
            ax.plot(
                xs, ys, label=opt, color=style["color"], marker=style["marker"],
                markersize=markersize, linewidth=1.6,
            )
            ...
            if std is not None:
                ...
                if xmax is not None:
                    ss = ss[mask]
                ax.fill_between(xs, ys - ss, ys + ss, color=style["color"], alpha=0.15, linewidth=0)
        ax.set_xscale("log")
        if xmax is not None:
            pos = [g for g in gammas if g > 0 and g <= xmax]
            if pos:
                ax.set_xlim(left=min(pos), right=xmax)
        if degradation:
            ax.axhline(0.0, color="black", linewidth=0.7, alpha=0.45)
        ax.set_title(f"chin={chin}  n={b['num_tokens']}", fontsize=10)
        ax.grid(True, which="both", alpha=0.3)
        ax.tick_params(labelsize=8)
        if rr == nrows - 1:
            ax.set_xlabel(r"$\gamma$  (weight noise)", fontsize=9)
        if cc == 0:
            ax.set_ylabel(r"$\Delta$ NLL (post$-$pre)" if degradation else "mean NLL", fontsize=9)
    ...
    fig.suptitle(f"Gaussian weight perturb on angle-bin $\\theta\\in[{bin_label}]^\\circ$  ({kind})", fontsize=12)
    ...
    path = os.path.join(out_dir, f"bin_{safe}", f"{tag}_vs_gamma_by_chin{suffix}.png")
...
def main():
    ...
    if not paths:
        raise SystemExit("No weight_angle_bin_perturb JSON files found.")
    print(f"Loading {len(paths)} runs")
    rows = collect(paths)

    bin_labels = sorted(
        {lab for r in rows for lab, b in (r.get("bins") or {}).items() if b.get("num_tokens", 0) > 0},
    ...
    for lab in bin_labels:
        ...
        plot_one_bin(rows, lab, args.out_dir, degradation=True, xmax=5e-2, markersize=1.0)
        plot_one_bin(rows, lab, args.out_dir, degradation=False, xmax=5e-2, markersize=1.0)
